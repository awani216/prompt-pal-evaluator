...
const DatasetUpload = ({ dataset, setDataset }: DatasetUploadProps) => {
  ...
  const validateDataset = (data: any) => {
    ...
    const emptyCount = data.rows.reduce((count: number, row: any) => {
      return count + data.headers.filter((header: string) => !row[header] || row[header].trim() === '').length;
    }, 0);
        ...
        <>
          ...
          >
            <div className="flex flex-col items-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Upload CSV Dataset</h3>
          ...
          <div className="border rounded-lg overflow-hidden bg-white">
            <div className="max-h-96 overflow-auto">
              <Table>
                ...
                <TableBody>
                  {dataset.rows.slice(0, 10).map((row: any, index: number) => (
                    <TableRow key={index}>
                      {dataset.headers.map((header: string) => (
